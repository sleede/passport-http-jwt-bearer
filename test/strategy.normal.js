  ...
  describe('handling a request with valid token in header', function() {
    ...
    before(function(done) {
      chai.passport.use(strategy)
        ...
        .req(function(req) {
          req.headers.authorization = 'Bearer ' + jwt.sign({}, secret, {subject: 'subject', expiresIn: '15m'});
        })
        .authenticate();
    });
  ...
  describe('handling a request with valid token in form-encoded body parameter', function() {
    ...
    before(function(done) {
      chai.passport.use(strategy)
        ...
        .req(function(req) {
          req.body = {};
          req.body.access_token = jwt.sign({}, secret, {subject: 'subject', expiresIn: '15m'});
        })
        .authenticate();
    });
  ...
  describe('handling a request with valid credential in URI query parameter', function() {
    ...
    before(function(done) {
      chai.passport.use(strategy)
        ...
        .req(function(req) {
          req.query = {};
          req.query.access_token = jwt.sign({}, secret, {subject: 'subject', expiresIn: '15m'});
        })
        .authenticate();
    });
  ...
  describe('handling a request with wrong token in header', function() {
    ...
    it('should fail with challenge when token is expired', function(done) {
      chai.passport.use(strategy)
        ...
        .req(function(req) {
          req.headers.authorization = 'Bearer ' + jwt.sign({}, secret, {subject: 'subject', expiresIn: '0'});
        })
        .authenticate();
    });
    ...
    it('should fail with challenge when token signature is invalid', function(done) {
      chai.passport.use(strategy)
        ...
        .req(function(req) {
          req.headers.authorization = 'Bearer ' + jwt.sign({}, secret + 'x', {subject: 'subject', expiresIn: '15m'});
        })
        .authenticate();
    });
    ...
    it('should fail with challenge when token signature is not signed', function(done) {
      chai.passport.use(strategy)
        ...
        .req(function(req) {
          req.headers.authorization = 'Bearer ' + jwt.sign({}, secret, {subject: 'subject', expiresIn: '15m', algorithm: 'none'});
        })
        .authenticate();
    });
    ...
    it('should fail with challenge when token audience does not match', function(done) {
      chai.passport.use(new Strategy(secret, {audience: 'foo'}, function(token, done) { done(null, false)}))
        ...
        .req(function(req) {
          req.headers.authorization = 'Bearer ' + jwt.sign({}, secret, {audience: 'bar', subject: 'subject', expiresIn: '15m'});
        })
        .authenticate();
    });
    ...
    it('should fail with challenge when token issuer does not match', function(done) {
      chai.passport.use(new Strategy(secret, {issuer: 'foo'}, function(token, done) { done(null, false)}))
        ...
        .req(function(req) {
          req.headers.authorization = 'Bearer ' + jwt.sign({}, secret, {issuer: 'bar', subject: 'subject', expiresIn: '15m'});
        })
        .authenticate();
    });
